...
`mcp_hello`, `mcp_impact`, `get_doc_for_file`, `find_clones`, `wake_up`, `search_by_environment`, `get_graph_report` (use get_god_nodes + get_architecture), `orchestrate` (use query_graph / kg_context / search_code), `search_by_requirement` (use get_traceability)
