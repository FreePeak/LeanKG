...
| M3 — Honest search | 2 | FR-HEA-02, FR-HEA-04 | **IN_PROGRESS** (FR-ZCP-05 bridge tier DONE 7d902461+3a68d571; FR-ZCP-06 freshness envelope LANDED on router + status paths — computed `engine_freshness` server.rs:331-347, injected :3611-3614 gated `mcp_status`, `retrieval{rung,reason,freshness}` router.rs:292-297; direct-action `get` responses still bare + tsvector FTS/RRF + reconciliation outstanding) |
...
| M7 — Embedding correctness | 1 | — | TODO (evidence verified 2026-09-07 on #279: no prefix mechanism, no model stamp, dim-only guard, partial batch validation) |
...
| FR-ZCP-06 | Freshness contract: `freshness: fresh\|possibly_stale\|cold` on every index-backed response; reconciliation off the query path — **envelope landed 2026-09-07 on router + status only** (computed `engine_freshness` server.rs:331-347 + injection :3611-3614 gated `tool_name == "mcp_status"`; `retrieval_block` call sites all in router.rs, none in handler.rs — direct-action `get` responses carry no envelope); reconciliation probe outstanding (watcher debounce/burst-pause only, watcher.rs:187-269) | P1 | M3 | FR-ZG-03, US-ZG-03 |
...
| FR-ZCP-11 | Embedding correctness ported from zvec-grep: pinned model catalog (commit revision + query/document prefixes), model-stamped vectors + hard rebuild guard, chunker-version coupling, 3-signal change detection, per-file atomic replace + truncation accounting, watcher reconciliation, single-flight indexing — **verified absent 2026-09-07** (evidence comment on #279): DirectEmbedder default path (build.rs:937-940), no prefix mechanism (models.rs:361-364, provider.rs:420-421), no revision/stamp (registry.rs:38-45; dim-only guard backend.rs:3279-3281), batch validation partial (models.rs:479, 485-490) | **P1** | M7 | FR-EMBED-R4 (supersedes the aspirational perf-only goal with a correctness contract) |
