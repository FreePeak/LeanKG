...
**Last synced:** 2026-09-14 — v4.11.3 **doctor-truth wave landed (#409, closes #406)**: five more fixes, all found by the loop running its own diagnostics back against the fixes — index-freshness compared against `code_elements` (24 zero-element files = un-clearable phantom "missing" → compare `code_files`); T0 manifest parents read as UNREADABLE in the fleet leg AND as `error` in the portfolio fan-out (now `MANIFEST` / `not_indexed`, claiming-but-broken stores still fault); `leankg gc` and the writer daemon's flush both bumped the watermark without refreshing the inventory snapshot (the watched project read `possibly_stale` forever → `RefreshInventory` on both paths; live proof: touch → flush → wm==snapshot, fresh). State now: **10 pass, 1 warn (the live writer's held lock — true by design), 0 fail; embedding coverage 9029/9029**. Third S3 child (`sketchpocalypse`, TS profile) indexed+embedded 244/244, fan-out spans 3 stores with attribution. PRD history was rebuilt mid-flight (#407 blob purge) — branch re-cut onto the new lineage, successor PR discipline applied.
**Previous syncs:** v4.11.2 waves one+two (S1 executed live — all five gates; 6 dogfood defects: embed budget/shrink, poison-item fallback, `leankg gc`, held-lock probe, test hermeticity, verb positional); v4.11.1 ship-surface; v4.11.0 self-host dogfood loop anchored (§3.10 + M10 + runbook, FR-SELF-01..04).
...
| Status | Count |
|--------|------:|
| IN_PROGRESS | 3 (FR-ZCP-01; **FR-SELF-02** — 11 engine defects found+fixed by the loop across #401/#405/#409; **FR-SELF-03** — three children live under `games/`) |
...
| M10 — Self-host dogfood loop | 4 | — | **IN_PROGRESS** (FR-SELF-01 **DONE** v4.11.2; FR-SELF-02 running — 11 defects fixed across #401/#405/#409, doctor now ends 10-pass/0-fail with coverage 9029/9029; FR-SELF-03 running — `games/` T0 + three embedded children, fan-out spans 3 stores; FR-SELF-04 is the steady state — see §M10 below) |
...
| FR-SELF-02 | Build LeanKG with LeanKG: every exported-symbol change informed by `impact`/`callers`, every close by tested-by/traceability, every session-open by `session_recall`; wrong answers filed as engine defects, fixed failing-test-first against this repo's data, re-indexed, re-asked | **P0** | **IN_PROGRESS 2026-09-14** — running since v4.11.2; cumulative output: 11 engine defects found by the loop and fixed failing-first (#401 embed budget + poison-item fallback + `gc` verb + held-lock probe + test hermeticity; #405 shrink-retry + embed-verb positional; #409/#406 index-freshness source, T0 classification on fleet AND fan-out, inventory refresh on gc + writer). The doctor-truth wave proved the meta-rule: re-run the loop's own diagnostics against each fix. |
...
*Last updated: 2026-09-14 (v4.11.3 doctor-truth wave (#409, closes #406): five honest-classification/refresh fixes landed — index-freshness vs bookkeeping, T0 parents MANIFEST/not_indexed on both surfaces, inventory refresh on gc + writer; live state 10 pass / 1 warn (held writer lock — true) / 0 fail, coverage 9029/9029; third S3 child live; PRD history rebuild (#407) handled via successor PR. Prior: v4.11.2 waves one+two — S1 five gates green, 6 defects fixed; v4.11.1 ship-surface)*
