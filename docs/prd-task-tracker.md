...
**Last synced:** 2026-09-14 — v4.12.1 **container-corpus**: the demo image built by #415's `Dockerfile` merged `cmd/` and `internal/` flat into `/demo/engine`, so the stage's `rm -rf .../internal/tstree` matched nothing and ~60 MB of vendored tree-sitter grammars were indexed into the public demo graph (#419; fixed + `test -d` layout guard). Previous sync: v4.12.0 **module-root** cut (#403 landed as #415 — `cmd/`/`internal/`/`go.mod` at the repository root, imports rewritten, `modtag` mirror retired, pb seed `go_package` length repaired), whose first root-tagged release **v0.33.0** was cut via the `Release-As:` trailer (#417/#418) and verified with four tarballs + no `go/` mirror tag.
...
| REL-SHIP-07 | Module root cut (#403): `git mv go/* .`, imports `github.com/FreePeak/LeanKG/go` → `github.com/FreePeak/LeanKG` (196 files), release/CI/Makefile/Dockerfile/.gitignore/.gitattributes/docs repointed; ttfv subshell `cd "$REPO_ROOT/go"` caught red by CI and fixed | Suite + tstree + tidy + guards green at root (PR #415); pristine-clone build of main clean; first root-tagged release **v0.33.0**: 4 tarballs, no `go/v` mirror tag, proxy + `go install @v0.33.0` + pkg.go.dev verified |
| REL-SHIP-08 | Demo corpus repaired (#419): each `COPY` names its destination so the `rm -rf /demo/engine/internal/tstree` exclusion works again, and the stage now asserts the layout before indexing | `docker build --target demo` → `cmd_layout_OK`, `internal_layout_OK`, `tstree_excluded_OK`; baked graph `4578` elements / `528` files (was `4771`/`555`); runtime image `/health` `{"ok":true}`, `/api/index/status` 4578 / 14889, `POST /api/query` → `fresh` hits, clusters read `engine/cmd/leankg` |
*Last updated: 2026-09-14 (v4.12.1 container-corpus — #419 fixed: demo stage flattened `cmd/`+`internal/` into `/demo/engine`, defeating the tstree exclusion; destinations named + layout guard, graph 4578/528 as documented.. Prior: v4.12.0-module-root shipped: #403 closed, PR #415 merged `a7a02c6`, **v0.33.0** cut via the `Release-As:` trailer path (#417/#418) — release-please's simple strategy filters squashed `refactor!` commits as non-user-facing, now pinned in the PRD §6 pipeline notes. Prior waves same as v4.11.4 vector-reclaim.)*
