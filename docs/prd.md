...
| [Graft](https://github.com/trailhq/Graft) (trailhq/NanoNets, TS, 5.9k★, audited @ `05760b0`) | Freshness as a query-path guarantee (stat-probe ~3 ms → bounded-wait refresh → answer, uncommitted edits included); intent-shape routing **before** capability selection; diff→symbol blast radius with recency-weighted `git log` owners + CI-safe 3-workflow PR comment pipeline; curated LLM concept synthesis (closed verb enum, forced-tool schema, empty-is-miss, `LlmFailureGate`, 15 s checkpoints, provenance inheritance); deterministic retrieval stack (BM25 name×3/path×2/body + personalized PageRank + bounded file pooling + scope-fusion fairness gates, zero embeddings); test-path de-rank ×0.35 query-aware; 1 MB vendor skip; measured token-savings header; hooks/statusline/advertisement-gating agent surface; structural invariants as CI gate | `#297` (LLM-meaning pipeline), `#290` (rung order), `#293` (test de-rank), `#291` (vendor skip), `#274` (freshness guarantee), `#272` (hooks), `#277` (fusion math), `#278` (invariants) — full analysis [`archive/analysis/graft-vs-leankg-2026-09-08.md`](archive/analysis/graft-vs-leankg-2026-09-08.md) |
...
| Storage | PostgreSQL + pgvector **or** SQLite/Cozo dual backend (`LEANKG_DB_ENGINE=sqlite\|postgres`; default sqlite when `LEANKG_PG_URL` unset — v4.4.0). PG: **one database, schema-per-project** (`leankg_p_<hex(canonical root)>`; per-connection `search_path` pin, per-schema migrations + HNSW); SQLite: one `.leankg/leankg.db` per project | Registry table + portfolio scope + cross-schema portfolio queries (FR-ZCP-09); fleet reconciliation (FR-ZCP-10) |
...
| Tools | Was 76/73 raw tools exact-count CI-pinned; v4.3.1 hard cutover → 1 tool; v4.4.0 → **3 tools** (`set`/`get`/`status`, `src/mcp/tools.rs:13,26,42`; exact-count test `:423`) with legacy capability names as per-tool `action`s | 3-tool envelope (FR-ZCP-03 end-state); published TTFV (FR-ZCP-12 T2); error code+fix catalog (FR-ZCP-12 T1, DONE); portfolio-scoped answers from T0 manifests |
...
| Default toolset surface | **3 tools** (`set`/`get`/`status`) — hard cutover, exact-count test (`src/mcp/tools.rs:423`); legacy capability names ride per-tool `action`s (`resolve_3tool`, `:117`) |
| Tool envelope (T3, v4.3.1→v4.4.0) | `list_tools().len() == 3` test-enforced; envelope resolved before RO-gate/write-lock/audit; unknown tool/action → catalog refusal with nearest-name fix |
| Storage | PostgreSQL + pgvector **or** SQLite/Cozo (`LEANKG_DB_ENGINE`, default sqlite absent `LEANKG_PG_URL`) — PG one database schema-per-project; the registry is the project SoT |
...
- **Graft (trailhq) deep-dive + comparison (2026-09-08, clone @ `05760b0` + five subsystem reads + line-by-line review of `ai/summarize.ts`/`ai/synthesize.ts`/`context/build.ts`/`context/node-file.ts`):** 14 ranked learnings (freshness guarantee, intent-before-rung, diff-blast+owners, inspectable artifacts, LLM-pipeline discipline, PPR rung, test de-rank, vendor skip, savings accounting, hooks, fusion math, invariants CI, failure gates, advertisement gating) + anti-cargo-cult list (LeanKG's semantic search, 100+ languages, org memory, server-grade multi-project, query-time LSP) → [`archive/analysis/graft-vs-leankg-2026-09-08.md`](archive/analysis/graft-vs-leankg-2026-09-08.md); findings filed as **#297** (high priority) and root-caused into **#290**

*Last updated: 2026-09-08 (Graft deep-dive + comparison landed: §1.1 steal row + §7 analysis link [`archive/analysis/graft-vs-leankg-2026-09-08.md`](archive/analysis/graft-vs-leankg-2026-09-08.md); issue #297 filed high-priority — graft LLM-meaning pipeline port (failure gate, content-hash resume, curated concept synthesis, provenance, notes-preservation); #290 root cause confirmed at `src/mcp/router.rs:486-521` — L3 arm never consults Lexical intent). Earlier: v4.4.1 — live-tested 3-tool SQLite server: dispatch unification across rmcp+JSON-RPC arms, Datalog repairs — import :put upsert / bracket nesting / fuzzy relation ref / positional→attribute rule forms / L3 hydration ports; issues #286–#288 open. v4.4.0 — 3-tool registry `set`/`get`/`status` + SQLite dual-backend; v4.3.1 — hard one-tool cutover: registry = `leankg_context` only, ~76 capabilities as `{verb}` envelope args, envelope resolved before RO-gate/write-lock/audit; v4.3.0 — one-tool degradation ladder (L0–L3, `retrieval` provenance) + first-run setup contract FR-ZCP-13 (auto/manual + `leankg add`) + FR-ZCP-05 bridge tier; v4.2.0 — measured-simplicity contract → FR-ZCP-12 T1/T2/T3 + M8 + D-2026-09-04-3; v4.1.1 — OMP memory-backend audit + zvec-grep embedding-correctness audit → FR-ZCP-11)*
