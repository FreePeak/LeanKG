...
| PG migrations 010 (org knowledge), 011 (context metrics) | **Gated tests exist but were never executed** (this environment had no local PostgreSQL/Docker; CI skips PG without `LEANKG_TEST_PG_URL`): `pg_orgknowledge_test.go` (incidents/knowledge_entries/service_metadata/env_snapshots — arrays, nullable BIGINTs, jsonb metadata) and `pg_metrics_test.go`. Their PG DDL is therefore validated by dialect review + the sqlite path only. Run `LEANKG_TEST_PG_URL=… go test ./internal/store/` where a server exists to close this. |
