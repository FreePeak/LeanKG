...
- **Duplicated logic collapsed:** index-freshness was derived twice (byte-identical in `core.freshness` and `doctor/fleetFreshness`) — now one `store.Freshness`; the savings percentage existed three times inside `internal/compress` — now one `savingsPercent`; the Postgres DSN ladder was re-implemented in two packages — now `projectcfg.PGURL`; four `sortedKeys` helpers became `slices.Sorted(maps.Keys(...))`.
