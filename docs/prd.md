...
| indexer extractor+call graph | 11k | `internal/index` + `internal/docindex` — regex extraction, 3-signal detection, calls/contains, watch-triggered reconcile | **PARTIAL — language breadth gap:** Go indexes **8 extensions** (go/rust/ts/tsx/js/jsx/py/md) vs the Rust engine's **17 core languages / 43 tree-sitter grammars** (registry enumerates 27). Closing it is W2 (tree-sitter), in tension with the `CGO_ENABLED=0` pure-Go build constraint; `examples/` holds 30 language demo projects the Go extractor does not yet parse |
