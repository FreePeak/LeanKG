...
*Last updated: 2026-09-17 (v4.13.0-memory-service — FR-ZCP-14: `serve --hindsight-compat` became a real service surface: `GET .../stats` (was 404, so the client printed "server unreachable" for a healthy server), `GET .../memories` list + `GET .../memories/{id}` read-by-id (wire id then document_id; unknown id = 404), `serve --memory-global` for a shared bank root, and the compat retain route folded into `auth`'s write gate — a Viewer could retain through the alias what Contributor was required to write natively. 46 packages ok, vet/gofmt clean. Prior: v4.12.2-hindsight-compat — #414 mounted omp's client wire byte-exact.)
