...
**Codebase Version:** 0.32.1 (Go engine at the repository root — module `github.com/FreePeak/LeanKG`, moved out of `go/` per #403; the Rust tree was removed in f7624143)
...
- **Gate:** full suite green at the root on both tag sets + `go mod tidy` no-op locally; CI + a pristine-clone build (global-gitignore trap) + the next release cut (no mirror job) verify it on the PR.
