...
| # | ID | Focus | Intent | Status |
|--:|----|-------|--------|--------|
| 1 | `FR-ZCP-11` | **P1** | Embedding correctness ported from zvec-grep: pinned model catalog (commit revision + query/document prefixes), model-stamped vectors + hard rebuild guard, chunker-version coupling, 3-signal change detection, per-file atomic replace + truncation accounting, watcher reconciliation, single-flight indexing | **IN_PROGRESS** — part 1 DONE (#351: pinned revisions in registry, per-collection model stamp {model_id, revision, dimensions, distance, provider}, hard rebuild guard in embed path; outstanding: 3-signal change detection, chunker_version coupling) |
