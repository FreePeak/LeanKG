...
**Status:** In progress — Phase 8 core DONE (T8.4 cleanup + T8.5 docs + T8.6 report); T8.1–8.3 (Docker/Render) deferred. Decisions D1–D5 resolved (2026-08-04).
...
> **Progress tracker (2026-08-05):** Phases 0–7 **done + verified** (parity spike, DbBackend trait + Arc threading, schema+migrations, SQL translator, vector swap, regression review, server semantics, COPY bulk embed; see §9). Phase 8 **core done** (T8.4 cleanup: cozo dep + DbBackend trait + CozoBackend shim + LEANKG_DB_ENGINE + persistent_cache/redis_store deleted, grep cozo=0, 936 lib tests green; T8.5 docs; T8.6 migration report). T8.1–8.3 (Docker/Render) **deferred**. Phase 9 not started. **Scope: focus 100% on MCP tools + core logic; WebUI/API-UI checks deferred (non-blocking).** Work happens on branch `worktree-leankg-pg-migration` (git worktree); do NOT touch the prod containers `leankg-leankg-1` (:9699) / `leankg-enterprise-cozoserver-1`. Dev Postgres runs as container `leankg-pg-phase0` (pgvector pg18, host port 5433).
...
| 8. Cleanup + docs + deploy | ✅ core done (T8.4/T8.5/T8.6); T8.1–8.3 deferred | T8.4 (code cleanup: delete cozo dep + DbBackend trait + CozoBackend shim + LEANKG_DB_ENGINE + persistent_cache/redis_store) **DONE** — grep `cozo`=0, 936 lib tests green, PG-only binary. T8.5 docs + T8.6 migration report **DONE**. T8.1–T8.3 (Docker/Render/compose) **DEFERRED** — ops, not a 0.20.0 blocker (scope 2026-08-05). |
| 9. Perf verification (index/embed/query on workspace-be) | ⬜ pending | T9.1–T9.6 + **T9.2b**, §4 Phase 9 — cold-index + embed **workspace-be** (`/Users/linh.doan/work/be`, ~371k functions), query latency vs cozo baselines, **end-to-end semantic_search quality at scale (recall@k ≥98% vs brute force, T9.2b)**, index review with `EXPLAIN`, autovacuum health, report `docs/analysis/pg-perf-large-codebase.md`. |
