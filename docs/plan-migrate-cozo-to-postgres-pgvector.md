...
| 5.5. Full regression review (all tools/features before → after) | ⬜ pending | T5.5.1–T5.5.5, §4 Phase 5.5 — full MCP tool sweep (85 tools), CLI sweep, WebUI Playwright test w/ screenshots (`ui-v2/e2e/`), perf guard (no tool >2x cozo), regression report `docs/analysis/pg-regression-report.md`. First task: fix the 13 test-compile breakages + content_hash/cleanup_old_metrics/graph:1728 cozo-vs-PG behavior flags. |
