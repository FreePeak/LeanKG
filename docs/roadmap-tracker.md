...
- Storage: **Postgres+pgvector only** (the legacy embedded engine crate removed from Cargo.toml). BUT Datalog survives as internal query IR: 238 `run_script()` call sites → runtime translator `src/db/pg/translate.rs` (4,301 LOC) + test fake `src/db/fake.rs` (1,402 LOC).
...
- APM audit: no third-party APM/APM telemetry exists anywhere in the codebase (only a competitor mention in one analysis doc). Nothing to remove.

### 3.3 Removal plan (from audit)
| Priority | Action |
|---|---|
| P0-a | Fix `tests/redundant_tools_matrix.rs` to current 76-tool reality |
| P0-b | Adopt/land the sql.rs seam from the legacy-engine-removal worktree (owns the machinery for full Datalog removal) |
| P0-c | Fix generated-doc lies ("stores data in the embedded graph DB") in doc/generator.rs + wiki.rs; purge the legacy integration/`rocksdb_backend` concepts from ontology YAMLs |
| P1-a | Delete orphaned db/mod.rs fns/structs (traceability quartet etc.), drop stale `#[allow(dead_code)]`, fix unused_mut warning |
| P1-b | Delete broken scripts/pg-cli-sweep.sh, prune .gitignore legacy entries, token_budget dead arms, delete stale v2-deprecation branch |
| P1-c | Sweep ~217 legacy-engine comment refs + ~57 datalog comment refs (ride along with waves) |
...
After hackathon: W8 (SQL-first seam waves) per the Datalog-removal SQL migration plan (see §0 companion documents); then ENT-1.
