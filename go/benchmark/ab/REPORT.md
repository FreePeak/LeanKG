...
- **Corpus:** 100 synthetic Go files, generated by the committed [`gen_corpus.py`](gen_corpus.py) (`python3 go/benchmark/ab/gen_corpus.py <dir>`). Two variants were used: the 3-symbol base corpus (2 funcs + 1 type; index + L1 rows) and the 4-symbol CHAINED corpus — identical generator with cross-file calls `next{i}` → `Handler{i+1}` (index-yield + impact rows). Fresh store per cold-index run; medians over the run counts in Raw runs.
...
- Corpus: `python3 go/benchmark/ab/gen_corpus.py /tmp/ab-corpus` (chained variant = the committed script; drop the `next{i}` call for the base variant).
- Go arm: `make go-build && go/benchmark/ab` (in-process) + `leankg index` / `leankg query` on the corpus (process).
