...
async fn test_check_call_graph_query() {
    ...
    let all_result = leankg::db::schema::run_script(graph.db(), all_calls_query, Default::default())
        .unwrap();
    ...
    let main_result = leankg::db::schema::run_script(graph.db(), &main_check, Default::default())
        .unwrap();
