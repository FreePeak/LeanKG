...
def resolve_sha(url: str, ref: str) -> str:
    """FR-ZCP-08: resolve a ref (tag/branch) to its pinned 40-hex commit SHA.

    Movable refs (master, stable/*) drift — the lock file freezes the corpus
    so a re-run in three months indexes the same commit.
    """
    out = subprocess.run(
        ["git", "ls-remote", url, ref],
        check=True, capture_output=True, text=True,
    ).stdout.split()
    # ls-remote <url> <ref> may match multiple lines (e.g. ^{} peeled tags);
    # prefer the peeled commit for annotated tags, else the first match.
    sha = ""
    for line_shas in out:
        sha = line_shas[0] if len(line_shas) == 40 else sha
    if not sha:
        raise RuntimeError(f"could not resolve {ref} at {url}")
    return sha


def git_clone_shallow(url: str, ref: str, target: Path) -> str:
    """Clone (or verify) and return the pinned 40-hex commit SHA."""
    if target.exists():
        ...
        try:
            ...
            wanted = resolve_sha(url, ref)
            if head == wanted:
                print(f"  [skip] {target.name} already at {wanted[:10]}")
                return wanted
            else:
...
def main() -> int:
    ...
    for entry in repos:
        ...
        try:
            sha = git_clone_shallow(url, ref, target)
            lock[slug] = {"url": url, "ref": ref, "sha": sha}
    ...
    # FR-ZCP-08: freeze the resolved SHAs so runs are pinned to a corpus.
    lock_path = args.target.parent / "repos.lock.yaml"
    if lock:
        lines = ["# Generated by clone_repos.py — do not edit.", "# Pinned corpus for benchmark reproducibility (FR-ZCP-08)."]
        for slug, info in lock.items():
            lines.append(f"{slug}:")
            lines.append(f"  url: {info['url']}")
            lines.append(f"  ref: {info['ref']}")
            lines.append(f"  sha: {info['sha']}")
